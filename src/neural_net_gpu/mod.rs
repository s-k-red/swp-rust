...
use crate::{neural_net_cpu::activation_function::SigmoidActivationFunction, config::MUTATION_RATE};

use self::activation_function::ActivationFunction;
pub mod matrix_utils_gpu;
mod activation_function_gpu;


#[derive(Debug, Clone, PartialEq)]
pub struct NeuralNetGpu{
...
impl NeuralNetGpu {
    ...
    fn init_weights(input_nodes:usize, hidden_nodes:usize, output_nodes:usize, hidden_layers:usize) -> Vec<Matrix<f32>> {
        ...
        for num in 0..hidden_layers+1 {
            if num == 0 {
                weights.push(matrix_utils_gpu::random_matrix(hidden_nodes, input_nodes, -1.0, 1.0));
            } else if num == hidden_layers {
                weights.push(matrix_utils_gpu::random_matrix(output_nodes, hidden_nodes, -1.0, 1.0));
            } else {
                weights.push(matrix_utils_gpu::random_matrix(hidden_nodes, hidden_nodes, -1.0, 1.0));
            }
        }
    ...
    fn init_biases(hidden_nodes:usize, output_nodes:usize, hidden_layers:usize) -> Vec<Matrix<f32>> {
        ...
        for num in 0..hidden_layers+1 {
            if num == hidden_layers {
                biases.push(matrix_utils_gpu::random_matrix(output_nodes, 1, -1.0, 1.0));
            } else {
                biases.push(matrix_utils_gpu::random_matrix(hidden_nodes, 1, -1.0, 1.0));
            }
        }
...
impl NeuralNetGpu {
    ...
    pub fn mutate(&mut self){
        ...
        for w in self.weights.as_slice() {
            weights.push(matrix_utils_gpu::mutate(w, MUTATION_RATE));
        }

        for b in self.biases.as_slice() {
            biases.push(matrix_utils_gpu::mutate(b, MUTATION_RATE));
        }
    ...
    pub fn save(&self){
        for (i, w) in self.weights.iter().enumerate() {
            ...
            matrix_utils_gpu::save_matrix(w.clone(), &name);
        }

        for (i, b)  in self.biases.iter().enumerate() {
            let name = format!("{}mut.txt", i);
            matrix_utils_gpu::save_matrix(b.clone(), &name);
        }
    }
